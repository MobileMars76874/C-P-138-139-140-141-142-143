...
function preload(){
music_ball_touched = loadSound("ball_touch_paddel.wav");
music_ball_missed = loadSound("missed.wav");
}
...
score_right_wrist = results[0].pose.keypoints[10];
}
}

function PG(){
status = "start";
document.getElementById("S").innerHTML = "Game is Loaded";
}
...
function move(){
  ...
  if (ball.y >= paddle1Y&& ball.y <= paddle1Y + paddle1Height) {
    ball.dx = -ball.dx+0.5;
    music_ball_touched.play(); 
  }
  else{
    pcscore++;
    reset();
    navigator.vibrate(100);
    music_ball_missed.play();
  }
}
if(pcscore ==4){
    fill("#FFA500");
